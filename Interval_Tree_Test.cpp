...
#define CATCH_CONFIG_RUNNER // Mark this file as the test-runner for catch 
#include "..\Interval_Tree\Unit_Test_Framework (Catch)\catch.hpp" // Include the catch unit test framework 

using namespace std;

typedef Interval_Tree<std::size_t, bool> intervalTree;
typedef intervalTree::interval interval;
typedef intervalTree::interval_vector intervalVector;

TEST_CASE( "Empty tree" )
{
    Interval_Tree<std::size_t, int> t;
    REQUIRE(t.findOverlapping(-1, 1).size() == 0);
}

TEST_CASE( "Singleton tree" )
{
    Interval_Tree<std::size_t, double> t
    {
        { { 1, 3, 5.5 } }, 
            1, 64, 512  
    };
    ...
    {
        typedef Interval_Tree<double, std::size_t> IT;
        IT t
        {
            { { 0.0, 1.0, 0 } }
        };
...
Interval<Scalar, Value> randomInterval(Scalar maxStart, Scalar maxLength, Scalar maxStop, const Value& value)
{
...
int main(int argc, char**argv)
{
    ...
    typedef Interval_Tree<int, bool> ITree;
    ...
    // Test set of target queries
    for (int i = 0; i < 5000; i++)
    {
        queries.push_back(randomInterval<int, bool>(100000, 100, 100000 + 1, true));
    }
